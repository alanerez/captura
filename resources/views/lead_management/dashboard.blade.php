                    ...
                    <div class="body">
                        <div class="row mb-5">
                            <div class="col-md-4 text-center">
                                ...
                                <h5 class="text-warning font-weight-bold" id="a-fees-value">${{ number_format($totalProfit['payables'], 2) }}</h5>
                            </div>
                            <div class="col-md-4 text-center">
                                <h6 id="a-revenue">Monthly Revenue</h6>
                                <h5 class="text-primary font-weight-bold" id="a-revenue-value">${{ number_format($totalProfit['income'], 2) }}</h5>
                            </div>
                            <div class="col-md-4 text-center">
                                <h6 id="a-fees">Total Sales</h6>
                                <h5 class="text-success font-weight-bold" id="t-profit-value">${{ number_format($totalProfit['profit'], 2) }}</h5>
                            </div>
                        </div>
...
@section('scripts')
    ...
    <script type="text/javascript">
        ...
        @endphp
        $('#t-profit-value').text('$' + '{{ number_format(\App\Http\Controllers\CashRegisterController::monthlyGross($month)['mprofit'], 2) }}')
        $('#a-revenue-value').text('$' + '{{ number_format(\App\Http\Controllers\CashRegisterController::monthlyGross($month)['income'], 2) }}')
        $('#a-fees-value').text('$' + '{{ number_format(\App\Http\Controllers\CashRegisterController::monthlyGross($month)['monthlyPayables'], 2) }}')
            ...
            datasets: [{
                ...
                data: [
                    ...
                    @php
                        ...
                        $first = $monthlyGross['mprofit'];
                    @endphp
                    {!! $fifth = $first !!},
                    @endfor
                    @for($k = $month+1; $k <=$month+4; $k++)
                    @php
                        $fifth += ($monthlyGross['income']) - $monthlyGross['legal'] - $monthlyGross['admin'];
                    @endphp
                    {!! $fifth !!},
                    @endfor
                    @php
                        $ytotal = ($monthlyGross['income'] * 5) - $monthlyGross['monthlyPayables'] - (($monthlyGross['legal'] + $monthlyGross['admin']) * 4);
                    @endphp
                    {!! $ytotal !!},
                ],
                fill: true,
                ...
                data: [
                    ...
                    @php
                        $monthlyGross = \App\Http\Controllers\CashRegisterController::monthlyGross($month);
                        $first = $monthlyGross['income'];
                    @endphp
                    {!! $fifth = $first !!},
                    @endfor
                    @for($k = $month+1; $k <=12; $k++)
                    @php
                        $fifth += $monthlyGross['income'];
                    @endphp
                    {!! $fifth !!},
                    @endfor
                    @php
                        $ytotal = ($monthlyGross['income'] * 5);
                    @endphp
                    {!! $ytotal !!},
                ],
            }, {
