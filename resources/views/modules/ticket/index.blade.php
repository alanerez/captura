...
@section('breadcrumbs')
{{ Breadcrumbs::render('ticket.index') }}
@endsection
@section('styles')
            ...
            <div class="card">
                ...
                <div class="mail-inbox">
                    <div class="mail-left collapse" id="email-nav">
                        <div class="mail-side">
                            <ul class="nav">
{{--                                <li class="{{ !isset($_GET['department_id']) ? 'active' : '' }}"><a href="{{ route('ticket.index') }}"><i class="fa fa-ticket"></i>All Tickets<span class="badge badge-primary float-right">0</span></a></li>--}}
                                <div class=""><h3>Departments</h3></div>
                                @foreach($departments as $department)
                                <li class="{{ @$_GET['department_id'] == $department->id ? 'active' : '' }}"><a href="{{ route('ticket.index', ['department_id' => $department->id])}}"><i class="fa fa-building-o"></i>{{ $department->name }}<span class="badge badge-primary float-right">0</span></a></li>
                                @endforeach
                            </ul>
                        </div>
                    </div>
                        ...
                        <div id="message-list">
                            <div class="header d-flex align-center">
                                ...
                                <form class="ml-auto">
                                    <div class="input-group">
                                        <input type="text" class="form-control" placeholder="Search Ticket" aria-label="Search Ticket" aria-describedby="search-mail">
                            ...
                            <div class="mail-action clearfix">
                                <div class="pull-left">
                                    ...
                                    @endif
                                    <div class="btn-group">
                                        @foreach($ticket_statuses as $k_status => $v_status)
                                        <a href="{{ route('ticket.index', array_merge(app('request')->request->all(), ['status_id' => $k_status]))}}" class="btn btn-outline-primary btn-sm"
                                            style="color: {{ $v_status['color'] }};"
                                            onmouseover="this.style.background='{{ $v_status['color'] }}'; this.style.color='{{ $v_status['text_color'] }}';"
                                            onmouseout="this.style.background=''; this.style.color='{{ $v_status['color'] }}';"
                                        >{{ $v_status['name'] }}</a>
                                        @endforeach
                                    </div>
                                </div>
                                <div class="pull-right ml-auto">
                                    <div class="pagination-email d-flex">
                                        <p>{{ $tickets->firstItem() }}-{{
                                            ($tickets->currentPage() * $tickets->perPage()) > $ticket_count ? $ticket_count : ($tickets->currentPage() * $tickets->perPage())
                                             }}/{{ $ticket_count }}
                                         </p>
                                        {!! $tickets->appends(request()->input())->render() !!}
                                    </div>
                                </div>
                            </div>
                            <div class="mail-list">
                                <ul class="list-unstyled">
                                    ...
                                    <li class="clearfix" {{-- style="background-color: rgba(242,245,245,0.8);" --}}>
                                        <div class="mail-detail-right">
                                            @php
                                            $child_newest = $ticket->childs->where('message_no', '!=', null)->last();

                                            if($child_newest == null) {
                                               $child_newest = $ticket;
                                            }

                                            @endphp
                                            <h6 class="sub">
                                            <a href="#ticket{{$ticket->id}}" class="mail-detail-expand" data-department="{{ $child_newest->department_id }}" data-messageid="{{ $child_newest->message_id }}" data-value="{{ json_encode($ticket) }}" data-id="{{ $ticket->id }}" data-date="{{ date('M d, Y g:i A',strtotime($child_newest->date)) }}">
                                                @foreach(@json_decode($ticket->from) as $from)
                                                {{ !empty($from->personal) ? $from->personal : $from->mail }}
                                                @endforeach
                                            </a>
                                            @if($ticket->tags != null)
                                            @foreach(@json_decode($ticket->tags) as $tag)
                                            <span class="badge badge-default mb-0">{{ @$tag }}</span>
                                            @endforeach
                                            @endif
                                            </h6>

                                            <p class="dep"><span class="m-r-10"  style="font-weight: bold; color: black;">[{{ $child_newest->subject }}]</span><span>{{ $ticket->text_body == false ? $child_newest->subject : $child_newest->text_body  }}</span></p>
                                            <span class="time">{{ date('M d, Y g:i A',strtotime($child_newest->date)) }}</span>
                                        </div>
