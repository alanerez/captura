...
@section('breadcrumbs')
{{ Breadcrumbs::render('ticket-priority.index') }}
@endsection
@section('styles')
        ...
        <div class="col-lg-12">
            <div class="card">
                <div class="header">
                    ...
                    <hr>
                    @permission('add-ticket-priority')
                    <button type="button" class="btn btn-outline-primary" data-toggle="modal" data-target="#add-modal">Add New Ticket Priority</button>
                    @endpermission
                </div>
                <div class="body">
...
@include('ticket_priority::includes._modal_edit')
@endpermission
@permission('add-ticket-priority')
@include('ticket_priority::includes._modal_add')
@endpermission
@endsection
@section('scripts')
...
<script>
    jQuery("#datatable").dataTable({
        processing: true,
        serverSide: true,
        ajax: '{{ route('ticket-priority.index') }}',
        columns : [
            {data: 'name'},
    ...
    $(document).on('click', '[id=btn-edit]', function(){
        var id = $(this).data('id');
        var show_route = '{{ route('ticket-priority.show', ':id') }}';
        var update_route = '{{ route('ticket-priority.update', ':id') }}';
