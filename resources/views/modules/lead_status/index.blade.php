...
@section('breadcrumbs')
{{ Breadcrumbs::render('lead-status.index') }}
@endsection
@section('styles')
        ...
        <div class="col-lg-12">
            <div class="card">
                <div class="header">
                    <h2>Lead Status</h2>
                    <hr>
                    @permission('add-lead-status')
                    <button type="button" class="btn btn-outline-primary" data-toggle="modal" data-target="#add-modal">Add New Lead Status</button>
                    @endpermission
                </div>
                <div class="body">
...
@include('lead_status::includes._modal_edit')
@endpermission
@permission('add-lead-status')
@include('lead_status::includes._modal_add')
@endpermission
@endsection
@section('scripts')
...
<script>
    ...
    $(document).on('click', '[id=btn-edit]', function(){
        var id = $(this).data('id');
        var show_route = '{{ route('lead-status.show', ':id') }}';
        var update_route = '{{ route('lead-status.update', ':id') }}';
